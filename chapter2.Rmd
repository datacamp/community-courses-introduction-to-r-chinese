...
你將利用 R 與資料分析的技巧來提升賭桌上的表現，並展開你的專業賭徒職涯。本章節會告訴你如何輕鬆地記錄下注過程以及如何針對這些記錄做簡單的分析，拉斯維加斯我們來囉！
...
這段程式首先宣告一個向量 `some_vector` 接著為裡面這兩個元素命名，第一個元素被命名為姓名 `Name` ，第二個元素被命名為職業 `Profession`。在 R Console 中印出 `some_vector`：
...
*** =instructions
- 編輯區已經完成 `poker_vector` 的贏錢記錄命名，接著請你對 `roulette_vector` 做一樣的事情。

*** =hint
使用 `names(roulette_vector)` 為 `roulette_vector` 命名，確認使用相同的向量來命名，記得英文字母的大小寫在 R 中是有差別的！
