...
title_meta  : 第二章
title       : 向量
description : "在第二章的課程中，我們要去拉斯維加斯一趟學習如何在R使用向量分析博弈的結果！完成本章節後，你將學會如何宣告向量、命名向量、從向量選取元素以及比較不同的向量。"

--- type:NormalExercise xp:100 skills:1 key:44ae1c0203a5a327a8b7670514bc5c5d07f05a5c
## 宣告向量

覺得自己運氣好嗎？你最好很有自信，因為本章節將帶你到傳說中的*統計學家天堂*、罪惡都市拉斯維加斯！

你將利用R與資料分析的技巧來提升賭桌上的表現，並展開你的專業賭徒職涯。本章節會告訴你如何輕鬆地記錄下注過程以及如何針對這些記錄做簡單的分析，拉斯維加斯我們來囉！
...
成為富翁的路上你會需要使用非常多的向量。向量是單一維度的陣列，可以包含數值、文字或者邏輯值。換句話說，向量是一種可以儲存資料的方法，例如你可以將在賭場中每天贏得的錢與每天損失的錢儲存在向量中。
...
*** =instructions 
宣告一個向量 `boolean_vector` 依序包含3個元素：`TRUE` 、 `FALSE` 與 `TRUE`。
...
經過一週在賭桌上的奮戰你還沒有為自己贏到一台法拉利跑車，於是你打算運用資料分析超能力來分析過去這週的輸贏記錄：
...
資料分析師需要對資料細節有清楚的認知，在前一個練習中我們宣告了過去一週的贏錢記錄，如果能夠在向量中顯示出哪個元素屬於哪一天，將會讓你對資料一目瞭然。
...
在前一個練習你可能對於不停輸入週間日感到無聊，其實有一個更有效率的方法，那就是把週間日指派給一個**變數**！

如同你記錄撲克牌與俄羅斯輪盤贏錢金額，你也可以宣告一個變數包含週間日的資訊，如此就可以重複使用它。
...
現在瞭解R如何做向量的數學運算，該是把法拉利買回來的時候了！首先你需要了解這一週每日的總獲利或總損失，而每日的總獲利或總損失就是將每天在撲克牌與俄羅斯輪盤的贏錢金額加總。
